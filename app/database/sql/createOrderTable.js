/**
 * @desc 创建订单相关表的sql。包好用户登录表，用户信息表，用户登录表
 * @author huangguoxin
 * @date 2019/9/5 16:00
 */

const createOrderMaster = `
 	CREATE TABLE IF NOT EXISTS lv_order_master(
	  ...
	  create_time TIMESTAMP NOT NULL DEFAULT '1971-01-01 00:00:00' COMMENT '下单时间',
...
const createOrderDetail = `
 	CREATE TABLE IF NOT EXISTS lv_order_detail(
...
const createOrderCart = `
	CREATE TABLE IF NOT EXISTS lv_order_cart(
	  cart_id INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '购物车ID',
	  user_id INT UNSIGNED NOT NULL COMMENT '用户ID',
	  goods_id INT UNSIGNED NOT NULL COMMENT '商品ID',
	  goods_number INT NOT NULL COMMENT '加入购物车商品数量',
	  price DECIMAL(8,2) NOT NULL COMMENT '商品价格',
	  add_time TIMESTAMP NOT NULL DEFAULT '1971-01-01 00:00:00' COMMENT '加入购物车时间',
	  modified_time TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最后修改时间',
	  PRIMARY KEY pk_cartid(cart_id)
	) ENGINE = innodb COMMENT '购物车表';
`;

const createWarehouseInfo = `
	CREATE TABLE IF NOT EXISTS lv_warehouse_info(
	  wid SMALLINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '仓库ID',
...
const createWarehouseGoods = `
	CREATE TABLE IF NOT EXISTS lv_warehouse_goods(
	  wg_id INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '商品库存ID',
	  goods_id INT UNSIGNED NOT NULL COMMENT '商品ID',
	  ...
	  modified_time TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最后修改时间',
	  PRIMARY KEY pk_wgid(wg_id)
	)ENGINE = innodb COMMENT '商品库存表'
`;


const createShippingInfo = `
	CREATE TABLE IF NOT EXISTS lv_shipping_info(
